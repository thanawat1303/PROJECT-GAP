...
const PageData = ({socket , addHref = false , HrefData , modify , auth , TabOn}) => {
    const [StatusPage , setStatus] = useState({
        status :    HrefData.get() === "data?plant" ? "plant" : 
    ...
    useEffect(()=>{
        modify(70 , 30 , ["หน้าแรก" , "ข้อมูลเพิ่มเติม"])
    ...
    const state = () => {
        const status =  HrefData.get() === "data?plant=pop" ? "plant" : 
    ...
    const ChangeStatus = async (statusClick) => {
        if(statusClick != StatusPage.status) {
            if(auth(true)) {
                ...
                HrefData.set(`data?${statusClick}=c`)
            }
        }
    }
        ...
        <section className="page-manage">
            <div className="menu-page">
                ...
                {StatusPage.status === "plant" ? 
                    <button className="bt-one" onClick={()=>ChangeStatus("station")}>แสดงรายการศูนย์</button> : 
                    <button className="bt-two" onClick={()=>ChangeStatus("plant")}>แสดงรายการชนิดพืช</button>
                }
