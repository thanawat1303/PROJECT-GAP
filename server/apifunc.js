...
const apifunc = {
  ...
  convertBuffer2Img : (data , props_img) => {
    if(typeof data === "object") return data.map(val=>{
      (val[props_img]) && (val[props_img] = val[props_img].toString());
      return val
    })
    else {
      (data[props_img]) && (data[props_img] = data[props_img].toString());
      return data
    }
  }
};
